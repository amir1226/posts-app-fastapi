...
def test_get_one_post_not_exist(authorized_client, test_posts):
    res = authorized_client.get("/posts/123456")
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("favorite pizza", "i love mexican", False),
    ("tallest skyscrapers", "wahoo", True),
    ("not published provided", "let's add default published", None)
])
...
def test_update_others_user_post(authorized_client, test_posts):
    data = {
        "title": "updated title",
...
def test_update_post_without_payload(authorized_client, test_posts):
    res = authorized_client.put(
        f"/posts/{test_posts[0].id}")
